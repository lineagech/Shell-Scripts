...
do
  ...
  then
    ...
    then
      ...
      if [[ "${?}" -ne 0 ]]
      then
        echo "The archive directory ${ARCHIVE} could not be created." >&2
        exit 1
      fi
    fi
  ...
  then
    ...
    # We dont want to tell the user that an account was deleted when it has not been.
    if [[ "${?}" -ne 0 ]]
    then
    ...
    echo
    chage -E 0 ${USERNAME}
    # Check to see if the chaneg command succeed.
    # We do not want to tell the user that an account was disabled when it has not been
    if [[ "${?}" -ne 0 ]]
    then
